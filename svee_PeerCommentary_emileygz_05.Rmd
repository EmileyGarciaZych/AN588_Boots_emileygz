...
Sam: I noticed you log transformed the data as a comment but it doesn't actually implement in the code. Did you mean to do this? I left an additional line of code that log transforms the data and I end up with slope and intercept values that match mine. Besides that, your code is clear so far! I didn't think to isolate the coefficients from the summary output in my own code, it would be a cleaner way to extract the data! 
```{r}
...
```
Sam: We used different methods! I'm not familiar with this package (I don't think we've used this in class?) but I think you did a good job of annotating everything and I can follow what you did. I'm slightly confused about the output - can you explain what V1 and V2 are?
...
Sam: Do you know why there are differences in the bootstrapped dataset vs the original dataset? I also used confint() to calculate my confidence intervals, can you explain the apply() function? Is it part of the boots package?

Sam: Overall I think you did a great job! Your code was organized and pretty well annotated. We used different code to answer the question so I just wasn't familiar with some of the packages or functions you used.
